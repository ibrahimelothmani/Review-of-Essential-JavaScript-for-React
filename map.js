...
function getHeroGenres(hero) {
    return hero.genres;
}

const genreID = hero.genres.map(genre => genre.id);
genreID;
const genreName = hero.genres.map(genre => genre.name);
genreName;
const genreDescription = hero.genres.map(genre => genre.description);
genreDescription;
const genreExamples = hero.genres.map(genre => genre.examples);
genreExamples;

const allGenres = getHeroGenres(hero).map(genre => {
    return {
        id: genre.id,
        name: genre.name,
